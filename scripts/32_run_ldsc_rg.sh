...
# Run separate asthma-outcome LDSC genetic correlations for the 11 outcomes
# with positive univariate h2. Eosinophilic disease is deliberately not run.
...
ASTHMA="${MUNGED_DIR}/asthma.sumstats.gz"
...
OUTPUT_DIR="${PROJECT_ROOT}/results/ldsc/rg"
...
if [[ ! -r "${ASTHMA}" ]]; then
  echo "Missing authoritative asthma summary statistics" >&2
  exit 1
fi

mkdir -p "${OUTPUT_DIR}"
printf 'outcome\texit_status\tconsole_log\tldsc_log\n' > "${RUN_STATUS}"

slugs=(allergic_rhinitis bronchiectasis bmi_irn nafld crp alt ast ggt hdl ldl triglycerides_fasting)
failures=0

for slug in "${slugs[@]}"; do
  outcome="${MUNGED_DIR}/${slug}.sumstats.gz"
...
echo "attempted_rg=${#slugs[@]}"
echo "failed_rg=${failures}"
exit "${failures}"
